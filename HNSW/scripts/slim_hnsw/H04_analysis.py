...
def generate_plots(runs_data, output_dir):
    ...
    for metric in metrics:
        ...
        if mean_val != 0:
            ...
            print(f"指標 {metric:<30} | 4次實驗均值: {mean_val:>10.4f} | 波動係數(CV): {cv:>7.4f} (越接近0越穩定)")

    plt.title("Experimental Fluctuation & Variance Analysis\n(Lines show runs; Shaded areas represent Min-Max Bounds)", fontsize=13, fontweight='bold', pad=15)
    plt.xticks(x_indices, x_labels)
    plt.ylabel("Relative Deviation (Value / Metric Mean)", fontsize=12)
    ...
    # 【修正 Bug 1】: 手動調高 Log 軸上限，留出 10 倍的空間給文字標籤，避免 167s 被切掉
    ax1.set_ylim(min(time_vals) * 0.5, max(time_vals) * 15)
    
    # 在長條圖上方精準標註數值
    for bar in bars:
        height = bar.get_height()
        # 針對 Log 軸，數值標籤位置用乘法調整比較不會跑版
    ...
    if 'QPS' in gmeans:
        ...
        # 【修正 Bug 2】: 橫跨整個 X 軸畫一條優雅的點虛線，並把點點定在正中央
        ...
        # 【修正 Bug 2】: 將 QPS 數值標記在圖表右上方的安全空白區域
...
if __name__ == '__main__':
    base_workspace = "/home/u08/workspace/HNSW"
    plots_output = os.path.join(base_workspace, "plots")
    
    print("正在收集與配對 4 份實驗日誌數據...")
